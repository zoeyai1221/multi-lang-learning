...
class Pacman(GameCharacter):
    ...
    def update(self):
        ...
        """Update the mouth angle here"""
        # Logic to control Pac-Man's mouth opening and closing
        if self.open_close == NUM_ZERO:
            self.mouth_angle += self.mouth_speed
            if (
                self.mouth_angle >= self.max_mouth_angle
                or self.mouth_angle <= self.min_mouth_angle
            ):
                self.mouth_speed *= -NUM_ONE
        ...
        # If the game is over and Pacman loses,
        # stop moving and keep mouth open at 360
        elif self.gc.pinky_wins:
            self.x_add = NUM_ZERO
            self.y_add = NUM_ZERO
            # make pacman's mouth open a little slower than usual
            # when it deads
            self.mouth_speed = NUM_ONE
            self.max_mouth_angle = FULL_CIRCLE_DEGREE
            # Ensure the mouth doesn't close again
            # by preventing the speed change
            if self.mouth_angle == self.max_mouth_angle:
                self.mouth_speed = NUM_ZERO
        ...
        # BEGIN CODE CHANGES
        # Pass Pacman instance to eat_dots method in Maze class
        self.maze.eat_dots(self)
        # END CODE CHANGES
